...
from tensorfilow.keras.layers import Flatten, Dense, Concatenate, Reshape, LSTM
...
dataset_dir = "/F:/2EEG_DateSet/DEAP/data_preprocessed_matlab/"
for i in range(len(short_name)):
    K.clear_session()
    start = time.time()
    print("\nprocessing: ", short_name[i], "......")
    file_path = os.path.join(dataset_dir, 's'+short_name[i])
